    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="LowerShelf(props.item)">下架</v-btn>
          <v-btn icon v-else @click="UpperShelf(props.item)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async editGoods(oldGoods) {  /*async： 异步转同步*/
        // 发起请求，查询商品详情和skus
      ...
      async deleteGoods(oldGoods){
          await this.$http.loadData("/item/delete/" + oldGoods.id);

      },
      async LowerShelf(oldGoods){
          await this.$http.loadData("item/lower/" + oldGoods.id);
          this.getDataFromServer();
      },
      async UpperShelf(oldGoods){
          await this.$http.loadData("item/upper/" + oldGoods.id);
          this.getDataFromServer();
      },
      closeWindow() {
        console.log(1)
